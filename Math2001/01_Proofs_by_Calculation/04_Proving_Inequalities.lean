...
example {x : ℤ} (hx : x ≥ 9) : x ^ 3 - 8 * x ^ 2 + 2 * x ≥ 3 :=
  calc
      x^3 - 8*x^2 + 2*x
    _ = x^2 * (x - 8) + 2*x := by ring
    _ >= x^2 * (9 - 8) + 2*x := by rel[hx]
    _ = x^2 + 2*x := by ring
    _ = x * x + 2 * x := by ring
    _ = x*(x + 2) := by ring
    _ >= 9 * (x + 2) := by rel[hx]
...
example {n : ℤ} (hn : n ≥ 10) : n ^ 4 - 2 * n ^ 2 > 3 * n ^ 3 :=
  calc
      n^4 - 2*n^2
    _ = (n^4 - 3*n^3 - 2*n^2) + 3*n^3 := by ring
    _ = n^2 * (n^2 - 3*n - 2) + 3*n^3 := by ring
    _ = n^2 * (n*n - 3*n - 2) + 3*n^3 := by ring
    _ >= n^2 * (10*n - 3*n - 2) + 3*n^3 := by rel[hn]
    _ = n^2 * (7*n - 2) + 3*n^3 := by ring
    _ >= n^2 * (7*10 - 2) + 3*n^3 := by rel[hn]
